## This code implements the RPCOVA method.
## It calculates the two estimates and gives standard errors
######################################################################
...
library(neuralnet)
library(slam)
library(gurobi)	# See directions for installation on their webpage
library(np)
library(mclust)
...
#####################################################################
#####################################################################
# x = data on covariates in matrix form
# z = treatment indicator in numeric form
# y = observed outcome in numeric form

# x0 = covariates data for the resistant population in matrix form
# y = observed outcome for the resistant population in numeric form
#####################################################################
#####################################################################
...
####################################### Create the evaluation set of X values
...
# newx = newx[sort(sample(1:nrow(newx),5000)),] Uncomment this if newx is too large?
...
Delta2x10.1 = Delta2x1


# This is to fix some out of bound error in the constrained optimization
Delta2x1[Delta2x1==0] = (mx1$mean - mx0$mean)[Delta2x1==0]
Delta2x = pmax(Delta2x1^2 - Delta2x2, 0)
...
## Create estimates and standard errors
# For plus
tauplusest = NULL
tauminusest = NULL
setauplus = NULL

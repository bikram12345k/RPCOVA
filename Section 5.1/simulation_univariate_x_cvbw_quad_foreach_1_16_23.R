...
z = rbinom(n,1, 1/(1+exp(-(.25*u+.5*u^2+.5*x-1))))
#rbinom(n,1, 1/(1+exp(-(.5*u+u^2+.5*x))))
y0 = 1+u*3+x+rnorm(n,0,.5)
taufn <- function(x) -x+x^2+.5
tau = taufn(x)

y = y0 + tau*z
nnfit = neuralnet::neuralnet(z~x, data=data.frame(z,x), hidden=8)
ok=1
...
yvec = as.numeric(y)
xdat = data.frame(x)
h1 <- npregbw(ydat=yvec,xdat=xdat,regtype='lc',bws=.25)
h1 <- npregbw(ydat=yvec,xdat=xdat,regtype='ll',bws=.25, bandwidth.compute = FALSE,regtype='ll')
...
h2 <- npregbw(ydat=rx^2,xdat=xdat,regtype='ll',bws=.25)
...
save.image(file='quad_12_19_22.RData')
